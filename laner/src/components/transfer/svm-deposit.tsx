...
interface SvmDepositProps {
  onBack: () => void;
  onActionChange: (action: 'deposit' | 'withdraw') => void;
  action: 'deposit' | 'withdraw';
}

const SvmDeposit: React.FC<SvmDepositProps> = ({ onBack, onActionChange, action }) => {
  const [amount, setAmount] = useState('1000');

  const handleTransfer = () => {
    // SVM deposit specific logic
    console.log(`SVM ${action}: ${amount} SOL on Solana`);
  };
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      <button 
        ...
        className="w-full rounded-lg py-4 font-bold uppercase mt-4 transition-colors bg-solana-green text-rich-black hover:bg-solana-green/90"
      >
